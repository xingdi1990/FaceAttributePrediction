...
from data.dataloadercelebA import *
...
datasetTrain = MyDataSet(filelist ='./datasets/celebA/celebATrain',
            transform=transforms.Compose([
                ToTensorDict(),
                NormalizeImageDict(['image'])
            ]))
dataLoaderTrain = data.DataLoader(datasetTrain, batch_size=50, shuffle=True, num_workers=2)
datasetTest = MyDataSet(filelist ='./datasets/celebA/celebAVal',
            transform=transforms.Compose([
                ToTensorDict(),
                NormalizeImageDict(['image'])
            ]))
dataLoaderTest = data.DataLoader(datasetTest, batch_size=50, shuffle=True, num_workers=2)
...
def train(epoch, model, loss_fn, optimizer, dataloader,log_interval=50):
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        optimizer.zero_grad()
        img, attr = Variable(sample_batched['image'].cuda()), \
                    Variable(sample_batched['attributes'].cuda(),requires_grad = False),
        pred_attr = model(img)
        print(img)
        # print(pred_attr)
        loss = loss_fn(pred_attr,attr)
        loss.backward()
        optimizer.step()
